...
use Simple\Orm\Entity\EntityInterface;
...
class Repository implements RepositoryInterface
{
    ...
    {
        try{
            return $this->entity->getCrud()->read($selectors, $conditions, $parameters, $optional);
        }catch(Throwable $throwable) {
            throw $throwable;
        }
    }
    ...
    public function findOneBy(array $conditions) : array
    {
        ...
        try{
            return $this->entity->getCrud()->read([], $conditions);
        }catch(Throwable $throwable) {
            throw $throwable;
        }
    }
    ...
    public function findObjectBy(array $conditions = [], array $selectors = []) : Object
    {
        $this->isArray($conditions);
        try{
            return $this->entity->getCrud()->get($selectors, $conditions);
        } catch(Throwable $throwable) {
            throw $throwable;
        }
    }
    ...
    public function findBySearch(array $selectors = [], array $conditions = [], array $parameters = [], array $optional = []) : array
    {
        $this->isArray($conditions);
        try{
            return $this->entity->getCrud()->search($selectors, $conditions, $parameters, $optional);
        }catch(Throwable $throwable) {
            throw $throwable;
        }
    }
    ...
    public function findByIdAndDelete(array $conditions) : bool
    {
        ...
        try{
            ...
            if ($result !=null && count($result) > 0) {
                $delete = $this->entity->getCrud()->delete($conditions);
                if ($delete) {
                    return true;
                }
            }
    ...
    public function findByIdAndUpdate(array $fields = [], int $id) : bool
    {
        $this->isArray($fields);
        try{
            $result = $this->findOneBy([$this->entity->getCrud()->getSchemaID() => $id]);
            if ($result !=null && count($result) > 0) {
                $params = (!empty($fields)) ? array_merge([$this->entity->getCrud()->getSchemaID() => $id], $fields) : $fields;
                $update = $this->entity->getCrud()->update($params, $this->entity->getCrud()->getSchemaID());
                if ($update) {
                    return true;
                }
            }
    ...
    private function getCurrentQueryStatus(Object $request, array $args)
    {
        ...
        if ($searchResults) {
            for ($i=0; $i < count($args['filter_by']); $i++) {
                ...
                $totalRecords = $this->entity->getCrud()->countRecords($conditions, $args['filter_by'][$i]);
            }
        } else if ($status) {
            $conditions = [$args['query'] => $status];
            $totalRecords = $this->entity->getCrud()->countRecords($conditions);
        } else {
            $conditions = [];
            $totalRecords = $this->entity->getCrud()->countRecords($conditions);
        }
